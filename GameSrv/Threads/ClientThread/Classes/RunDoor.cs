...
namespace RandM.GameSrv {
    public class RunDoor {
            ...
            try {
                ...
                if (((_ClientThread.NodeInfo.Door.Platform == OSUtils.Platform.Linux) && OSUtils.IsUnix) || ((_ClientThread.NodeInfo.Door.Platform == OSUtils.Platform.Windows) && OSUtils.IsWindows)) {
                    ...
                    if (ProcessUtils.Is64BitOperatingSystem) {
                        ...
                        RunDoorSBBSEXEC(TranslateCLS(_ClientThread.NodeInfo.Door.Command), TranslateCLS(_ClientThread.NodeInfo.Door.Parameters), _ClientThread.NodeInfo.Door.ForceQuitDelay);
                    }
        ...
        private unsafe void RunDoorSBBSEXEC(string command, string parameters, int forceQuitDelay) {
            if (Helpers.Debug) _ClientThread.UpdateStatus("DEBUG: SBBSEXECNT launching " + command + " " + parameters);
            ...
            string EnvFile = StringUtils.PathCombine(ProcessUtils.StartupPath, "node" + _ClientThread.NodeInfo.Node.ToString(), "dosxtrn.env");
            string RetFile = StringUtils.PathCombine(ProcessUtils.StartupPath, "node" + _ClientThread.NodeInfo.Node.ToString(), "dosxtrn.ret");
            string W32DoorFile = StringUtils.PathCombine(ProcessUtils.StartupPath, "node" + _ClientThread.NodeInfo.Node.ToString(), "w32door.run");
            ...
            try {
                ...
                while (!_ClientThread.QuitThread()) // NB: Was simply _Stop before
                {
                    ...
                    if (!_ClientThread.NodeInfo.Connection.Connected) {
                        ...
                        _ClientThread.UpdateStatus("Process still active after waiting " + forceQuitDelay.ToString() + " seconds");
                        return;
                    }
                    ...
                    if (!DataTransferred) {
                        ...
                        if (LoopsSinceIO >= 3) {
                            ...
                            if (File.Exists(W32DoorFile)) {
                                try {
                                    if (Helpers.Debug) _ClientThread.UpdateStatus("DEBUG: w32door.run found");
